...
-- 오답노트
ALTER TABLE omori.notes
	ADD CONSTRAINT FK_selected_answer_TO_notes -- 사용자가 선택한 답 -> 오답노트
		FOREIGN KEY (
			sa_no -- 선택한답번호
		)
		REFERENCES omori.selected_answer ( -- 사용자가 선택한 답
			sa_no -- 선택한답번호
		)on delete cascade on update cascade;
...
-- 문제 1번의 객관식 보기
select * from testexample where tq_no = 1 order by te_small_no;
-- 문제 1번의 보기와 답
...
select * from grade where uid = 'test1' order by g_date desc, g_no desc;
select * from grade;
