...
public class GradeController {
	...
	public String gradeGET(HttpServletRequest req, Model model) throws Exception{
		...
		if (testNameList != null) {
			...
			List<String> tempList = gradeService.selectGradeDate(user.getUid(), testNameList.get(0).getTno());
			if(testNameList.get(0).getTname().contains("정보처리기사")){
				for (String date : tempList) {
					List<GradeVO> gradeList = gradeService.selectListGradeByDate(user.getUid(), testNameList.get(0).getTno(), date);
